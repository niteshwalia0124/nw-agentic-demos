...
def tool_calculate_risk_score(
    ...
    Args:
        ...
        state: Indian state abbreviation for regional risk (e.g., MH, KA, DL, GJ).
...
def tool_calculate_premium(
    ...
    """Calculate the final premium for an insurance policy in Indian Rupees (₹).

    Args:
        product_id: The insurance product ID (e.g., AUTO_COMPREHENSIVE).
        base_monthly_premium: Base premium from product catalog in ₹ (e.g., 850.0 for DriveShield Comprehensive).
        risk_multiplier: Risk composite multiplier from risk calculator.
        coverage_level: standard, enhanced, or premium tier.
        deductible: Selected deductible amount in ₹.
    ...
    Returns:
        Premium breakdown with base, adjustments, discounts, and final amount — all values in ₹.
    """
    ...
    # Tag output with currency for downstream agents
    result["currency"] = "INR"
